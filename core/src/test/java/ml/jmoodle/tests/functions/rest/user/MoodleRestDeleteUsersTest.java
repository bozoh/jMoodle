...
public class MoodleRestDeleteUsersTest  {
	...
	public void verify_since_version_test() throws MoodleWSFucntionException {
		MoodleRestDeleteUsers cc = (MoodleRestDeleteUsers) MoodleWSFunctionFactory.getFunction(
			MoodleWSFunctions.CORE_USER_DELETE_USERS, mc
		);
	...
	public void verify_function_name_above_or_equal_2_2_0_test () throws MoodleWSFucntionException {
		...
		assertThat(cc.getFunctionName()).isEqualTo("core_user_delete_users");
	}
	...
	public void verify_function_name_lower_2_2_0_test () throws MoodleWSFucntionException {
		...
		assertThat(cc.getFunctionName()).isEqualTo("moodle_user_delete_users");
	}
	...
	public void verify_function_data_test() throws UnsupportedEncodingException, MoodleWSFucntionException  {
		...
		Set<MoodleUser> entities = new HashSet<>(Fixture.from(MoodleUser.class).gimme(15, "MoodleRestDeleteUsersFunction"));
		...
		doFunctionDataAssertions(dataString, entities);
	}

	private void doFunctionDataAssertions(String dataString, Set<MoodleUser> entities) {
		int i = 0;
		for (MoodleUser e : entities) {
			assertThat(dataString).contains("userids["+i+"]=");
			assertThat(dataString).contains(e.getId().toString());
			i++;
		}
	}
