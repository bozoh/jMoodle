...
public class MoodleNote implements Serializable {
	 ...
	 *            identify the note.
	 */
	public MoodleNote(Long userId, String publishState, Long courseId, String text, String format,
			String clientNoteId) {
		this.userId = userId;
		this.publishState = publishState;
		this.courseId = courseId;
		this.text = text;
		if (format.toLowerCase().equals(FORMAT_TEXT) || format.toLowerCase().equals(FORMAT_PLAIN))
			this.format = DescriptionFormat.PLAIN;
		if (format.toLowerCase().equals(FORMAT_HTML))
			this.format = DescriptionFormat.HTML;
		if (format.toLowerCase().equals(FORMAT_MOODLE))
			this.format = DescriptionFormat.MOODLE;
		if (format.toLowerCase().equals(FORMAT_MARKDOWN))
			this.format = DescriptionFormat.MARKDOWN;
		this.clientNoteId = clientNoteId;
	}

	public MoodleNote(Long userId, String publishState, Long courseId, String text, DescriptionFormat format,
			String clientNoteId) {
		this.userId = userId;
		this.publishState = publishState;
		this.courseId = courseId;
	...
	public void setMoodleNoteField(String nodeName, String content) {
		...
		if (nodeName.equals("noteid") || nodeName.equals("id"))
			setNoteId(Long.valueOf(content));
		if (nodeName.equals("courseid")) {
			courseId = Long.parseLong(content);
		}
		...
		if (nodeName.equals("created")) {
			created = Long.parseLong(content);
		}
		if (nodeName.equals("content")) {
			this.text = content;
		}
		if (nodeName.equals("lastmodified")) {
			lastModified = Long.parseLong(content);
		}
		if (nodeName.equals("usermodified")) {
			userModified = Long.parseLong(content);
		}
		if (nodeName.equals("publishedstate")) {
