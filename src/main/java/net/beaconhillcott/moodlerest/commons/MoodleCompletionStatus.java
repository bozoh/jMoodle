...
public class MoodleCompletionStatus implements Serializable {
	...
	private Integer state = null; // int //completion state value: 0 means
									// incomplete, 1 complete, 2 complete pass,
									// 3 complete fail
	...
	public void setField(String nodeName, String content) {
		if (nodeName.equals("cmid")) {
			setCmId(Long.parseLong(content));
		}
		if (nodeName.equals("modname")) {
			setModName(content);
		}
		if (nodeName.equals("instance")) {
			setInstance(Long.parseLong(content));
		}
		if (nodeName.equals("state")) {
			setState(Integer.parseInt(content));
		}
		if (nodeName.equals("timecompleted")) {
			setInstance(Long.parseLong(content));
		}
		if (nodeName.equals("tracking")) {
			setTracking(Long.parseLong(content));
		}
	}
